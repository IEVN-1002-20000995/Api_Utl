...
def lista_alumno(mat):
    try:
        alumno=leer_alumnos_bd(mat)
        if alumno!=None:

            return jsonify({'alumnos':alumno, 'mensaje':'Alumno encontrado',
                            'exito':True})
        else:
...
def registrar_alumno():
    try:
        alumno=leer_alumnos_bd(request.json['matricula'])
        if alumno!=None:
...
@app.route('/alumnos/<mat>', methods=['PUT'])       #put, manjeo de servicios donde se notificara que se modificara algo
